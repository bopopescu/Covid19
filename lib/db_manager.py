...
class db_manager:
    ...
    def menu(self):
        ...
        while not exit:
            ...
            if choice == 1:
                ...
                search = self.__search_by_top()
                print(search)
            elif choice == 5:
                recoverd = self.__search_by_top_recovered()
                print(recoverd)
            elif choice == 0:
                exit = True
                print("Bye!")
    ...
    def __search_by_top_recovered(self):
        self.__cursor.execute(
            "SELECT Country, TotalRecovered FROM `countries`  ORDER BY  TotalRecovered DESC LIMIT 10")
        recoverd = self.__cursor.fetchall()
        for item in recoverd:
            print(item[0], ": ", item[1], '\n') 
        # return recoverd            
